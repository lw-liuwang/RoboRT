...
std::vector<uint8_t> GgufReader::read_convert(const char * name, ggml_type target, bool gemma_norm) {
    ...
    if (t->type == GGML_TYPE_F32) {
        ...
        std::fprintf(stderr, "pi05: tensor %s has unsupported type %d\n", name, static_cast<int>(t->type));
        return {};
    }

    // Gemma RMSNorm weights are stored as (w - 1) in the GGUF; restore the +1 bias.
    if (gemma_norm) {
    ...
    if (ggml_is_quantized(target)) {
        ...
        if (n_per_row % blck != 0) {
            std::fprintf(stderr, "pi05: cannot quantize %s to %s: ne0=%lld not divisible by block=%lld\n", name,
        ...
        if (written != qbytes) {
            std::fprintf(stderr, "pi05: quantized byte mismatch for %s (%zu vs %zu)\n", name, written, qbytes);
            return {};
        }
        return out;
    }
    std::fprintf(stderr, "pi05: unsupported resident type %d for %s\n", static_cast<int>(target), name);
    return {};
}
...
bool GgufReader::fetch_rows_f32(const char *                 name,
    ...
    if (t->ne[0] != cols || t->ne[2] != 1 || t->ne[3] != 1) {
        std::fprintf(stderr, "pi05: %s shape unfit for row-fetch\n", name);
        return false;
    }
    ...
    for (size_t k = 0; k < row_ids.size(); ++k) {
        ...
        if (r < 0 || r >= rows) {
            std::fprintf(stderr, "pi05: row %d out of range for %s\n", r, name);
            return false;
        }
