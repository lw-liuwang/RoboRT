...
struct clip_ctx {
    ...
    // VisPruner: last attention weights (saved from clip_graph for token pruning)
    std::vector<float> last_attn_data;
    int                last_attn_n_patches = 0;
...
ggml_tensor * clip_graph::build_attn(
    ...
    if (flash_attn_type == CLIP_FLASH_ATTN_TYPE_ENABLED) {
        ...
        // VisPruner: save attention weights at the target layer
        if (il == attn_save_layer) {
            char attn_name[32];
            std::snprintf(attn_name, sizeof(attn_name), "attn_w_%d", il);
            ggml_set_name(kq, attn_name);
            saved_attn = kq;
        }
...
static ggml_cgraph * clip_image_build_graph(clip_ctx * ctx, const clip_image_f32_batch & imgs) {
    ...
    // VisPruner: save attention from the last ViT layer for token pruning
    builder->attn_save_layer = builder->n_layer - 1;
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs_c_ptr, float * vec) {
    ...
    // PALIGEMMA (pi0.5's projector) does NOT support batched encode; the
    // batch machinery is IDEFICS3-specific.
    ...
    // VisPruner: extract saved attention weights for token pruning
    {
        // attn_save_layer = n_layer - 1, name format = "attn_w_%d"
        const int attn_layer = ctx->model.hparams.n_layer - 1;
        char attn_name[32];
        std::snprintf(attn_name, sizeof(attn_name), "attn_w_%d", attn_layer);
        ggml_tensor * attn = ggml_graph_get_tensor(gf, attn_name);
        if (attn) {
            // attention shape: (N, N, H, 1) where N = n_patches, H = n_head
            const int n_patches_attn = attn->ne[0]; // number of patches (ne[0] = N)
            const int n_heads_attn   = attn->ne[2];
            const int nelem = (int) ggml_nelements(attn);
            ctx->last_attn_data.resize(nelem);
            ggml_backend_tensor_get(attn, ctx->last_attn_data.data(), 0, ggml_nbytes(attn));
            ctx->last_attn_n_patches = n_patches_attn;
            LOG_INF("%s: VisPruner: saved attention weights from layer %d, shape [%lld, %lld, %lld] (%d elements)\n",
                    __func__, attn_layer, attn->ne[0], attn->ne[1], attn->ne[2], nelem);
        } else {
            LOG_WRN("%s: VisPruner: attention tensor '%s' not found (flash attention enabled?)\n", __func__, attn_name);
            ctx->last_attn_data.clear();
            ctx->last_attn_n_patches = 0;
        }
    }
...
// VisPruner: access saved attention weights from the last image encoding call
const float * clip_get_last_attn_data(const struct clip_ctx * ctx, int * n_patches, int * n_heads) {
    if (ctx->last_attn_data.empty()) {
        *n_patches = 0;
        *n_heads   = 0;
        return nullptr;
    }
    *n_patches = ctx->last_attn_n_patches;
    // shape was (N, N, H, 1), so n_heads = ne[2] of the original tensor.
    // We recover it: total elements = N * N * H, so H = total / (N * N)
    *n_heads   = (int) (ctx->last_attn_data.size() / ((size_t) ctx->last_attn_n_patches * ctx->last_attn_n_patches));
    return ctx->last_attn_data.data();
}
