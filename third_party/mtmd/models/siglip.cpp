...
ggml_cgraph * clip_graph_siglip::build() {
    ...
    if (proj_type == PROJECTOR_TYPE_GEMMA3) {
        ...
        // NOTE: no extra scaling here. Reference (openpi/lerobot) uses the projector
        // output as-is; the language embeddings are scaled by sqrt(hidden) inside
        // GemmaTextScaledWordEmbedding and pi05.cpp applies lang_scale = sqrt(hidden).
        // Scaling the vision features by 1/sqrt(ne[0]) shrank them ~45x and made the
        // VLM effectively blind to images (eval success 0%).
